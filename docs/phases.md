...
## Fase 21 — Gestión de secretos por entorno (dev/qa/prod)

Cierra la deuda de seguridad arrastrada desde las fases tempranas: credenciales en claro en `appsettings.json` (connection strings con `Password=postgres`, `CliClientSecret: cli-dev-secret`, usuarios demo seedeados por defecto, `GF_SECURITY_ADMIN_PASSWORD` con default `-admin` en el compose de prod).

**Qué se añadió**:

- **`appsettings.json` sin secretos (×5 servicios: Auth, Catalog, Orders, Inventory, OrderSaga.Worker)**: eliminadas todas las credenciales en claro. Quedan **placeholders `CHANGE_ME`** (fail-fast: si el entorno no los sobreescribe, la conexión a BD/registro del cliente `cli` falla al arrancar) en `ConnectionStrings.*`; en **Auth** además `OpenIddict__CliClientSecret: CHANGE_ME` (el default `cli-dev-secret` ya no viaja en el archivo base) y `AuthUsers.Users: []` (sin usuarios demo).
- **`appsettings.Development.json` nuevo por servicio (×5)**: credenciales de desarrollo — connection strings con `Password=postgres` (los 5), en Auth además los usuarios demo `admin/admin123` y `customer/customer123` y `CliClientSecret: cli-dev-secret`. **Solo se cargan con `ASPNETCORE_ENVIRONMENT=Development`** (`dotnet run`/tests); el archivo de Auth incluye una propiedad `"_comment"` explicando el propósito. Con esto el quickstart y los tests siguen funcionando sin tocar código.
- **`docker/.env.prod.example` (nuevo)**: plantilla de secretos para prod/qa documentada — **obligatorias** (fail-fast, sin default en el compose a propósito): `POSTGRES_PASSWORD`, `RABBITMQ_USER`, `RABBITMQ_PASSWORD`, `CLI_CLIENT_SECRET`, `GRAFANA_ADMIN_PASSWORD`; **opcionales** con default: `POSTGRES_USER`, `BOOKSTORE_PUBLIC_ISSUER`, `GRAFANA_ALERT_WEBHOOK_URL`. `docker/.env` está en `.gitignore` (patrón `*.env`).
- **`docker-compose.prod.yml` parametrizado**: todos los secretos pasan a `${VAR}` — `POSTGRES_PASSWORD: ${POSTGRES_PASSWORD}` (sin default), las 5 connection strings con `Password=${POSTGRES_PASSWORD}`, **RabbitMQ** con `RABBITMQ_DEFAULT_USER: ${RABBITMQ_USER}` y `RABBITMQ_DEFAULT_PASS: ${RABBITMQ_PASSWORD}`, **orders/inventory/sagaworker** con `RabbitMQ__Host: "rabbitmq://${RABBITMQ_USER}:${RABBITMQ_PASSWORD}@rabbitmq"`, **auth** con `OpenIddict__CliClientSecret: ${CLI_CLIENT_SECRET}` y **grafana** con `GF_SECURITY_ADMIN_PASSWORD: ${GRAFANA_ADMIN_PASSWORD}` (se **eliminó el default `:-admin`** de la Fase 20).
- **`.github/workflows/cd.yml`**: el paso Deploy pasa a la sesión remota los **6 secretos** desde GitHub Secrets (`POSTGRES_PASSWORD`, `RABBITMQ_USER`, `RABBITMQ_PASSWORD`, `CLI_CLIENT_SECRET`, `GRAFANA_ADMIN_PASSWORD`, `GRAFANA_ALERT_WEBHOOK_URL`) por `env` + `envs:` del `appleboy/ssh-action`; el compose los interpola **sin `.env` en el servidor**.
- **Comportamiento de seguridad**: en prod/qa **ya no hay usuarios demo por defecto** (se movieron a Development); se pueden inyectar opcionalmente por env (`AuthUsers__Users__0__Username`, `AuthUsers__Users__0__Password`, `AuthUsers__Users__0__Role`, …). Las credenciales `admin/admin123`/`customer/customer123` quedan explícitamente **solo para desarrollo local**.

**Qué no se añadió y por qué**:

- **No se adoptó un gestor de secrets dedicado** (Vault/secretos de Docker Swarm/cloud KMS): para el alcance del proyecto basta el patrón estándar de ingeniería de infraestructura 12-factor — **secretos fuera del código y del compose**, variables de entorno como única fuente en runtime, y fail-fast si no están definidas. Un secreto manager real queda como evolución si el despliegue crece (multi-entorno/multi-región).
- **`sagaworker` no recibió usuarios ni secretos extra**: como worker sin API no tiene connection string de Auth ni `CliClientSecret`; solo `ConnectionStrings__OrderSagaDb` y `RabbitMQ__Host` parametrizados.
- **No se añadió *user-secrets* como obligatorio en dev**: se menciona como opcional porque `appsettings.Development.json` cubre el 100 % del flujo local sin él; quien quiera overrides locales puede usar `dotnet user-secrets` (misma precedencia de ASP.NET Core).

**Verificación**: `docker compose config` interpola **sin valores secretos en el archivo resultante** (las `${VAR}` se resuelven en runtime); **94 tests en verde** (los integration tests arrancan su propio entorno y config); **E2E en prod con `.env` local**: libro creado vía API, order `Pending → Shipped`, token OIDC emitido con `CLI_CLIENT_SECRET` por env, RabbitMQ y Postgres conectados con credenciales personalizadas (no las de dev); `cd.yml` revisado con los 6 `envs:`.
