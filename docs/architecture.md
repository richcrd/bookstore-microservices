...
- `cd.yml`: al crear un tag `v*` → SSH al servidor → `docker compose build` + `up -d` (requiere secrets `SERVER_HOST`, `SERVER_USER`, `SSH_PRIVATE_KEY`, `GRAFANA_ALERT_WEBHOOK_URL` y `vars.DEPLOY_DIR`); `GRAFANA_ALERT_WEBHOOK_URL` se pasa al servidor por `env` + `envs:` del `appleboy/ssh-action` para que el compose de prod interpole el contact point de alertas de Grafana.
